...
\begin{align*}
ln(Y)/\mu_i,\ \sigma \sim N(\mu_i,\ \sigma) \\ \mu_i=\beta_0 + \beta_1\cdot (t+\delta) \\ \beta_0 =2.7081 \\ \beta_1 \sim N^+(0.16,\ 0.075) \\ \delta \sim U(1.5, \ 2.5) \\ \sigma \sim N^+(0,\ 0.2 )
\end{align*}
...
Guido observa que esta primera observación parece acotar levemente el intervalo de tiempo en el que puede decir con una buena certeza que Sergio murió. Ahora Guido puede decir con la misma credibilidad de antes que Sergio falleció entre las `r predictiva_post1[1]` y las `r predictiva_post1[2]`, calculado a partir de la distribución a posteriori de $\delta$, que se muestra en la figura @fig-cadenas2. Esto no parece mucho, pero le sirve a Guido para saber que va por buen camino, lo cual lo motiva a seguir.
...
Guido ahora concluye con una certeza del 90% que Sergio pereció entre las `r predictiva_post2[1]` y las `r predictiva_post2[2]`, calculado a partir de la distribución a posteriori de $\delta$, que se muestra en la figura @fig-cadenas5.
...
Hasta ahora todo iba bien, las cadenas aún convergían con todos los $\hat R$ iguales a 1, pero al ver en la figura @fig-cadenas8 la densidad estimada del parámetro $\delta$ sospechaba que algo no encajaba. Sin embargo, decidió seguir adelante.
...
Con estos resultados, Sergio pudo haber fenecido entre las `r predictiva_post3[1]` y las `r predictiva_post3[2]`, un intervalo de tiempo mayor que el que se había obtenido anteriormente con la misma credibilidad, calculado a partir de la distribución a posteriori de $\delta$, que se muestra en la figura @fig-cadenas8.
