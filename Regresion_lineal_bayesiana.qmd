...
pred_prior <- data.frame(
  prior_beta_1 = abs(rnorm(n, 0.16, 0.025)),
  prior_d = abs(rnorm(n, 2, 0.2)),
  prior_sigma = abs(rnorm(n,0,0.15))
  ) 
...
# save(file = "Stan_models/Resultados_modelos.RData")
...
quantile(x = pred_prior$prior_d, probs = c(0.05, 0.95))
quantile(x = df_posterior$d, probs = c(0.05, 0.95))
quantile(x = df_posterior_2$d, probs = c(0.05, 0.95))
quantile(x = df_posterior_3$d, probs = c(0.05, 0.95))
