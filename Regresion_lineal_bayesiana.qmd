...
differ
for (i in grilla_t) {
  vec <- pred_prior$prior_beta_0 + pred_prior$prior_beta_1*i
  
}

calc_diff_prom <- function(t, data) {
  return(exp(data$prior_beta_0 + data$prior_beta_1 * t))
}


diff_prom <- sapply(grilla_t, calc_diff_prom, pred_prior)

dim(diff_prom)

data.frame(t = grilla_t,
           media = colMeans(diff_prom)) |> 
  ggplot()+
  aes(x = t, y = media)+
  geom_point()

graf <- log_peso_prom |> `colnames<-`(grilla_t)
