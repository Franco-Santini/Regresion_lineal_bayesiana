...
$$
Donde $T_{amb}$ es la temperatura ambiente en donde se encuentra el cuerpo, en este caso 22 °C, $r$ es una constante que determina el ritmo de enfriamiento y $T(t)$ es la función que describe la temperatura del cadaver en función del tiempo después de la muerte.

Una posible función $T(t)$ que satisface esta ecuación diferencial es la siguiente:

\begin{align*}
T(t) = T_{\text{amb}} + (T_i - T_{\text{amb}})e^{-rt} \Rightarrow \frac{d T(t)}{d t} = -r(T_i - T_{\text{amb}}) \cdot e^{-rt} \Rightarrow \\ \Rightarrow r(T_{\text{amb}} - T_{\text{amb}})\cdot e^{-rt} - r(T_i - T_{\text{amb}}) \cdot e^{-rt} = r \{T_{\text{amb}} - T_{\text{amb}} - (T_i - T_{\text{amb}})e^{-rt} \} = \\ = r \{T_{\text{amb}} - \underbrace{[T_{\text{amb}} + (T_i - T_{\text{amb}})e^{-rt}]}_{T(t)} \} = r [T_{\text{amb}} - T(t)]
\end{align*}

Algunos valores posibles para la constante de enfriamiento r son:


```{r grafico funcion temp amb 23 1}
...
```{r grafico funcion temp amb 23 2}
...
$$
T(t) - T_{\text{amb}} = T_{\text{diff}} e^{-rt} \Rightarrow \\ ln(T(t) - T_{\text{amb}}) = ln(T_{\text{diff}} e^{-rt}) \Rightarrow \underbrace{ln(T_{\text{diff}})}_{\beta_0} - \underbrace{r}_{\beta_1}t = \beta_0 - \beta_1 \cdot t
$$
...
\Rightarrow ln(T(t+\delta) - T_{\text{amb}}) = ln(T_{\text{diff}} e^{-r(t+\delta)}) \Rightarrow \\ 
\Rightarrow \underbrace{ln(T_{\text{diff}})}_{\beta_0} - \underbrace{r}_{\beta_1}(t+\delta) = \beta_0 - \beta_1 \cdot (t+\delta)
$$
El modelo que se usa para estimar la hora de muerte de Sergio resulta:
...
$$
ln(T(0)-T_{amb}) = \beta_0 - \beta_1 \cdot (0) \Rightarrow
...
Con respecto al parámetro $\beta_1$, considerando la hora a la que estima que murió Sergio y la temperatura a la que fue encontrado, observa lo siguiente.

```{r grafico funcion temp amb 22 1}
r1 <- function_T(t = seq(0,60, length.out = 1000),
           t_amb = 22,
           t_ini = 37,
           r = 0.1375)
...
```{r grafico funcion temp amb 22 2}
...
Con respecto al parámetro $\sigma$, cree que un termómetro puede fallar a lo sumo en 2 °C, por lo que postula una distribución media normal, dado que $\sigma$ no puede tomar valores negativos, con un desvío de 0.25. 
...
\begin{align*}
ln(Y)/\mu_i,\ \sigma \sim N(\mu_i,\ \sigma) \\ \mu_i=\beta_0 - \beta_1\cdot (t+\delta) \\ \beta_0 =2.7081 \\ \beta_1 \sim N^+(0.16,\ 0.02) \\ \delta \sim U(1.5, \ 2.5) \\ \sigma \sim N^+(0,\ 0.25)
\end{align*}
...
```{r grafico bandas prior 1}
load(file = "Stan_models/Resultados_modelos.RData")

datos <- data.frame(
  t = c(0,1.5, 6.75),
  diff = c(10.8,8.5, 1.7),
  Observacion = factor(c("06:45hs | 32.8 ºC", "08:15hs | 30.5 ºC", "13:30hs | 23.7 ºC"), 
                       levels = c("06:45hs | 32.8 ºC", "08:15hs | 30.5 ºC", "13:30hs | 23.7 ºC"))
)
...
ggplot(data_y_pred) +
  ...
  geom_line(
    aes(t, y), 
    color = "firebrick") +
  labs(x = "Tiempo", y = expression(T(t+delta)-T[amb])) +
  scale_x_continuous(breaks = seq(-4,7,1), limits = c(-4,7))
...
Después de que la computadora de bajos recursos del departamento de la Policía Científica estuviera encendida durante días, Guido consiguió correr los modelos en Stan.
...
colores <- c("dodgerblue", "gold", "lightgreen", "pink2")
```
...
```{r graficos cadenas 3}
#| label: fig-cadenas3
#| fig-cap: "Cadenas de Markov para $\\sigma$"
#| fig-subcap: 
#|   - "Traza"
#|   - "Densidad"
#| layout-ncol: 2

trace_plot(df_posterior, 3, colores)

density_plot(df_posterior, 3,  colores)
```
...
ggplot(data_mu) +
  ...
  geom_line(
    ...
    color = "firebrick") + 
  geom_point(
    data = datos[1,],mapping = aes(x = t, y = log(diff), color = Observacion), size = 1.5) +
  scale_color_manual(values = c("gold")) +
  labs(x = "Tiempo", y = expression(mu[i]), color = "Hora | Temperatura") +
  scale_x_continuous(breaks = seq(-4,7,1), limits = c(-4,7)) +
...
ggplot(data_y) +
  ...
  geom_line(
    ...
    color = "firebrick") +
  geom_point(
    data = datos[1,],mapping = aes(x = t, y = diff, color = Observacion), size = 1.5) +
  scale_color_manual(values = c("gold")) +
  labs(x = "Tiempo", y = expression(T(t+delta)-T[amb]), color = "Hora | Temperatura") +
  scale_x_continuous(breaks = seq(-4,7,1), limits = c(-4,7)) +
  theme(legend.position = "top")
...
Guido observa que esta primera observación parece acotar levemente el intervalo de tiempo en el que puede decir con una buena certeza que Sergio murió. Ahora él, puede decir con la misma credibilidad de antes que Sergio falleció entre las `r predictiva_post1[1]` y las `r predictiva_post1[2]`, calculado a partir de la distribución a posteriori de $\delta$, que se muestra en la @fig-cadenas2. Esto no parece mucho, pero le sirve a Guido para saber que va por buen camino, lo cuál lo motiva a seguir.

Guido procede a seguir añadiendo datos a su modelo, con lo que usa la información disponible hasta las 10hs. 
...
De igual forma que con un solo dato, todos los $\hat R$ son iguales a 1, indicando que las cadenas convergen a la misma distribución para todos los parámetros. Esto se puede ver en los siguientes gráficos de traza y densidad para cada uno de los parámetros.

```{r graficos cadenas 4}
#| label: fig-cadenas4
#| fig-cap: "Cadenas de Markov para $\\beta_1$"
#| fig-subcap: 
#|   - "Traza"
...
trace_plot(df_posterior_2, 1, colores)

density_plot(df_posterior_2, 1,  colores)
```

```{r graficos cadenas 5}
#| label: fig-cadenas5
#| fig-cap: "Cadenas de Markov para $\\delta$"
#| fig-subcap: 
#|   - "Traza"
...
trace_plot(df_posterior_2, 2, colores)

density_plot(df_posterior_2, 2,  colores)
```

```{r graficos cadenas 6}
#| label: fig-cadenas6
#| fig-cap: "Cadenas de Markov para $\\sigma$"
#| fig-subcap: 
#|   - "Traza"
...
trace_plot(df_posterior_2, 3, colores)

density_plot(df_posterior_2, 3,  colores)
```
...
ggplot(data_mu2) +
  ...
  geom_line(
    ...
    color = "firebrick") + 
  geom_point(
    data = datos[1:2,],mapping = aes(x = t, y = log(diff), color = Observacion), size = 1.5) +
  scale_color_manual(values = c("gold", "lightgreen")) +
  labs(x = "Tiempo", y = expression(mu[i]), color = "Hora | Temperatura") +
  scale_x_continuous(breaks = seq(-4,7,1), limits = c(-4,7)) +
  theme(legend.position = "top")
...
ggplot(data_y2) +
  ...
  geom_line(
    ...
    color = "firebrick") + 
  geom_point(
    data = datos[1:2,],mapping = aes(x = t, y = diff, color = Observacion), size = 1.5) +
  scale_color_manual(values = c("gold", "lightgreen")) +
  labs(x = "Tiempo", y = expression(T(t+delta)-T[amb]), color = "Hora | Temperatura") +
  scale_x_continuous(breaks = seq(-4,7,1), limits = c(-4,7)) +
  theme(legend.position = "top")
...
Guido ahora concluye con una certeza del 90% que Sergio pereció entre las `r predictiva_post2[1]` y las `r predictiva_post2[2]`, calculado a partir de la distribución a posteriori de $\delta$, que se muestra en la @fig-cadenas5.
...
trace_plot(df_posterior_3, 1, colores)

density_plot(df_posterior_3, 1,  colores)
```

```{r graficos cadenas 8}
#| label: fig-cadenas8
#| fig-cap: "Cadenas de Markov para $\\delta$"
#| fig-subcap: 
#|   - "Traza"
...
trace_plot(df_posterior_3, 2, colores)

density_plot(df_posterior_3, 2,  colores)
```

```{r graficos cadenas 9}
#| label: fig-cadenas9
#| fig-cap: "Cadenas de Markov para $\\sigma$"
#| fig-subcap: 
#|   - "Traza"
...
trace_plot(df_posterior_3, 3, colores)

density_plot(df_posterior_3, 3,  colores)
```


Hasta el momento, todo iba bien, las cadenas aún convergían con todos los $\hat R$ iguales a 1, pero al ver en la @fig-cadenas8 la densidad estimada del parámetro $\delta$ sospechaba que algo no encajaba. Sin embargo, decidió seguir adelante.
...
ggplot(data_mu3) +
  ...
  geom_line(
    ...
    color = "firebrick") + 
  geom_point(
    data = datos,mapping = aes(x = t, y = log(diff), color = Observacion), size = 1.5) +
  scale_color_manual(values = c("gold", "lightgreen", "dodgerblue")) +
  labs(x = "Tiempo", y = expression(mu[i]), color = "Hora | Temperatura") +
  scale_x_continuous(breaks = seq(-4,7,1), limits = c(-4,7)) +
  theme(legend.position = "top")
...
ggplot(data_y3) +
  ...
  geom_line(
    ...
    color = "firebrick") + 
  geom_point(
    data = datos,mapping = aes(x = t, y = diff, color = Observacion), size = 1.5) +
  scale_color_manual(values = c("gold", "lightgreen", "dodgerblue")) +
  labs(x = "Tiempo", y = expression(T(t+delta)-T[amb]), color = "Hora | Temperatura") +
  scale_x_continuous(breaks = seq(-4,7,1), limits = c(-4,7)) +
  theme(legend.position = "top")
...
hora_mas_probable <- dec_a_hex(6.75 - mean(df_posterior_2$d))
```

Con estos resultados, Sergio pudo haber fenecido entre las `r predictiva_post3[1]` y las `r predictiva_post3[2]`, un intervalo de tiempo mayor que el que se había obtenido anteriormente con la misma credibilidad, calculado a partir de la distribución a posteriori de $\delta$, que se muestra en la @fig-cadenas8.
...
Guido decide informarle a los policías que Sergio murió entre las `r predictiva_post2[1]` y las `r predictiva_post2[2]`, considerando a las `r hora_mas_probable` como la hora más probable de la muerte, ignorando el último dato, dado que consideró que este empeoraba las estimaciones por haberse obtenido en condiciones diferentes a las del lugar del crimen. La policía forense agradece a Guido por su esfuerzo y se pondrán a investigar más detenidamente el caso con la información brindada. Guido antes de despedirse le recomendó a los policias verificar el [repositorio](https://github.com/Franco-Santini/Regresion_lineal_bayesiana) donde realizó toda su investigación.
