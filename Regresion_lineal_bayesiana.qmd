...
$$
ln(T(t) - T_{\text{amb}}) = \underbrace{ln(T_{\text{diff}})}_{\beta_0} + \underbrace{(-r)}_{\beta_1}t = \beta_0 + \beta_1 \cdot (t)
$$
...
pred_prior <- data.frame(
  prior_beta_1 = -abs(rnorm(n, -0.175, 0.05)),
  prior_sigma = abs(rnorm(n,0,0.3))
  ) 


calc_diff_prom <- function(t, data) {
  return((beta_0 + data$prior_beta_1 * t))
}
...
```{r prediccion 7am 2}
# intercepto_promedio <- mean()
pendiente_promedio <- mean(df_posterior$beta1)

t_dist <- (log(15) - 2.3795461341)/df_posterior$beta1
...
calc_diff_prom_posterior <- function(t, data) {
  return((2.3795461341 + data$beta1 * t))
}
...
mean(t_dist_2 >= -2 & t_dist_2 <= 0) # Con valores + y -
